...
_데모 GIF 추가 예정 — 위 라이브 링크에서 직접 확인할 수 있습니다._

<!-- 데모 GIF 녹화 후(호가창 변동 + VI 배너 발동 + 리플레이 버튼 클릭) 아래 주석을 해제:
![데모](docs/demo.gif)
-->
...
### 왜 이 프로젝트를 만들었나
"주식 주문은 어떻게 체결되는가"를 글로 아는 것과 직접 구현해 보는 것은 전혀 달랐다. 가격-시간 우선 원칙, 부분 체결, 시장가/지정가, 동시 주문 처리, 거래정지(VI)까지 — 자본시장 매매 인프라의 핵심을 작게라도 직접 만들어 보며 "백엔드 시스템을 어떻게 설계하는가"를 보여주는 포트폴리오로 삼았다.

### 가장 고민한 설계 결정
- **동시성**: 가장 먼저 부딪힌 문제. lock으로 오더북을 감쌀 수도 있었지만, "주문을 큐에 넣고 단일 스레드가 혼자 처리"하는 구조를 택했다. lock이 아예 없으니 race condition을 디버깅할 일이 없고, 매칭 순서가 결정적이라 **이벤트 리플레이로 동일 결과를 재현**할 수 있다는 보너스까지 따라왔다. 동시성 문제를 "없애는" 설계의 힘을 체감했다.
- **VI를 매칭 스레드 안에 둔 것**: 거래정지 상태를 별도 컴포넌트가 관리하면 또 동기화가 필요했을 것이다. 단일 스레드 모델 덕분에 정지/해제/일괄체결을 평범한 필드 조작으로 처리할 수 있었다 — 좋은 기반 설계가 이후 기능을 얼마나 쉽게 만드는지 배웠다.
- **트리거 판정 버그**: VI를 "첫 체결가" 기준으로 만들었더니 대형 주문이 호가를 쓸어도 절대 발동하지 않았다. 첫 체결가는 항상 최우선호가(기준가 근처)였기 때문. "가장 깊은 체결가"로 바꿔 해결했는데, 도메인을 이해해야 올바른 트리거를 설계할 수 있다는 걸 배운 지점이다.

### 정직하게 기록한 한계
- 성능 테스트의 persist 단계는 객체 구성 비용만 모사하고 **실제 PostgreSQL I/O는 포함하지 않는다** — 수치를 부풀리지 않으려 README에 명시했다.
- VI는 KRX의 단일가매매를 단순화(순차 일괄체결)했고, 리플레이는 순수 가격-시간 우선이라 VI 구간에서 원본과 차이가 난다(그 차이 자체를 VI의 효과로 해석해 보여준다).
- 단일 종목·단일 인스턴스. 다중 인스턴스 확장(Redis 등)은 의도적으로 범위 밖에 뒀다.

### 더 했다면
- 실제 DB I/O를 포함한 부하 테스트와 백프레셔 동작 관찰
- 리플레이를 VI까지 포함해 완전 재현 + step-by-step 시각화
- 주문 취소/정정 비율 등 더 현실적인 트레이더 행동 모델

### 면접에서 답할 수 있는 질문
"왜 TreeMap인가 / 동시 주문은 어떻게 처리했나 / 테이블을 왜 3개로 분리했나 / 이벤트 로그를 왜 분리했나 / VI를 어떻게 race 없이 구현했나 / 리플레이가 왜 결정적인가" — 모두 위 **설계 결정** 섹션에 근거와 함께 정리해 두었다.
